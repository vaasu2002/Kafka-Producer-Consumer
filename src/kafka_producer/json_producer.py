...
FILE_PATH = "sample_data\kafka-sensor-topic\sensor_failure.csv"
...
def product_data_using_file(topic,file_path):
    schema_str = Generic.get_schema_to_produce_consume_data(file_path=file_path) # STEP 2
    schema_registry_conf = schema_config() # STEP 3
    schema_registry_client = SchemaRegistryClient(schema_registry_conf)

    string_serializer = StringSerializer('utf_8')
    json_serializer = JSONSerializer(schema_str, schema_registry_client, instance_to_dict)

    producer = Producer(sasl_conf()) # STEP 4
